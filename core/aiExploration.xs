...
minInterval 30
{
	if (cvOkToExplore == false || gIslandMap == false)
	{
		xsDisableSelf();
		return;
	}
	...
	case cIslandExploreModeSearch:
	{
		...
		if (gStartOnDifferentIslands == false && isEnemyInSight == true)
			isEnemyInSight = enemyAreaGroupID < 0 && (kbGetAge() < cAge2 || (xsGetTime() - gLastAttackMissionTime >= gAttackMissionInterval));
	...
	case cIslandExploreModeStart:
	{
		...
		int scoutType = -1;
		int planID = -1;
		int unitType = cUnitTypeAbstractFindScout;

		if (kbUnitCount(cMyID, unitType, cUnitStateAlive) == 0)
			unitType = cUnitTypeAbstractInfantry;
		if (kbUnitCount(cMyID, unitType, cUnitStateAlive) == 0)
			return;

		unitQueryID = createSimpleUnitQuery(unitType, cMyID, cUnitStateAlive);
		...
		aiPlanAddUnitType(islandExplorePlan, unitType, 1, 1, 1);
...
minInterval 15
{
	...
	if (gStartOnDifferentIslands == true)
	{
		...
		// Only use war ships for exploration if we have no fishing ships and no combat going on.
...
void exploreScoutShoreline(int explorePlanID = -1)
{
	...
	debugExploration("closestFishPosition: " + closestFishPosition + ", closestFishAreaID:" + closestFishAreaID + ", closestFishAreaCenter: " + closestFishAreaCenter);
	...
	for (i = 0; < numberBorderAreas)
	{
		...
		{
			distanceToMainBase = getDistance(mainBasePosition, kbAreaGetCenter(neighbouringAreaID));
	...
	debugExploration("neighbouringLandAreaID1: " + neighbouringLandAreaID1 + ", neighbouringLandAreaCenter1: " + neighbouringLandAreaCenter1);
	debugExploration("neighbouringLandAreaID2: " + neighbouringLandAreaID2 + ", neighbouringLandAreaCenter2: " + neighbouringLandAreaCenter2);
	...
	int fishLandDistance1 = getDistance(closestFishPosition, neighbouringLandAreaCenter1);
	...
	for (i = 1; < amountOfSteps1)
	{
		...
		debugExploration("i: " + i + ", canPathTestVector1: " + canPathTestVector1 + " with type: " + kbAreaGetType(kbAreaGetIDByPosition(canPathTestVector1)));
	...
	if (neighbouringLandAreaCenter2 != cInvalidVector)
	{
		...
		int fishLandDistance2 = getDistance(closestFishPosition, neighbouringLandAreaCenter2);
		...
		for (i = 1; < amountOfSteps2)
		{
			...
			debugExploration("i: " + i + ", canPathTestVector2: " + canPathTestVector2 + " with type: " + kbAreaGetType(kbAreaGetIDByPosition(canPathTestVector2)));
...
minInterval 10
{
	...
	int() createExplorerControlPlan = []() -> int {
		...
		aiPlanSetVariableVector(
			controlPlan, cCombatPlanTargetPoint, 0, kbBaseGetMilitaryGatherPoint(cMyID, mainBaseID));
		...
		if (civIsEuropean() && cMyCiv != cCivDEAmericans && cMyCiv != cCivDEMexicans)
